...
def training_schedule(model, train_data_loader, val_data_loader, optimizer, max_communication, device, hydra_output_dir, loss=torch.nn.CrossEntropyLoss(),  plot=True, save_model=True):
    ...
    try:
        for epoch in range(1, 1000):
            ...
            avg_val_loss, avg_val_accuracy, val_stats = validation_phase(model, val_data_loader, loss, device, plot)
            ...
            if avg_val_accuracy >= best_val_accuracy:
                ...
                top_imp_frac = val_stats.get("mode_alloc_top_imp_frac")
                if top_imp_frac is None:
                    top_imp_frac = val_stats.get("stream_alloc_top_imp_frac")
                if top_imp_frac is None:
                ...
                best_stats = {
                    ...
                    "tokens_sent": val_stats.get("tokens_sent", train_stats.get("tokens_sent", 0)),
                    "symbols_sent": val_stats.get("mimo_L", train_stats.get("mimo_L", 0)),
                    "symbol_rate": val_stats.get("rate_symbols", train_stats.get("rate_symbols", 0.0))
                }
                # Add all other numerical stats to best_stats
                for k, v in train_stats.items(): best_stats[f"best_train_{k}"] = v
                for k, v in val_stats.items(): best_stats[f"best_val_{k}"] = v
